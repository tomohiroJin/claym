...
### 2.2 GPU / ローカル LLM（Ollama）

[Ollama](https://ollama.com/) CLI がプリインストールされており、ローカル LLM をコンテナ内で実行できます。NVIDIA GPU を持つ環境では GPU アクセラレーションが利用可能です。macOS ではホスト側の Ollama に接続する方式を推奨します。

- GPU セットアップの詳細は [docs/gpu-setup.md](docs/gpu-setup.md) を参照してください
- 起動スクリプト: `bash scripts/gpu/start-ollama.sh`
...
| Codex CLI | `prompts/*.md` | `.codex/prompts/`（共有） / `~/.codex/prompts/`（個人） | カスタムプロンプト（slash コマンド） |
...
- Codex CLI のカスタムプロンプト: `.codex/prompts/`（共有）や `~/.codex/prompts/`（個人）を編集
...
### 5.4 devcontainer.local.json の活用
...
- MCPサーバーの追加と最適化
- ドキュメントの多言語対応
- Ollama + ローカル LLM の活用パターンの拡充（MCP 連携、エージェント統合等）
