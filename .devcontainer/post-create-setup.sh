...
────────────────────────────────────────
MCP 登録の初期設定が完了しました（Claude Code 用）。

■ 使い始める
  - 端末で `claude chat` を実行し、各 MCP が利用できるか確認してください。
...
■ 解除・再登録（Claude）
  - 解除: `claude mcp remove <name>`
  - 再登録: 本スクリプトを再実行（重複は自動的に無視）

※ Codex CLI / Gemini CLI への MCP 自動登録はデフォルト無効です。
   これらが公式に `mcp add` 相当を提供した場合は、同様の関数呼び出しを追記してください。
────────────────────────────────────────
EOF
