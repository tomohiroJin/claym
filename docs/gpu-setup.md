...
| 推奨 devcontainer | **Claym AI Dev (GPU)** | **Claym AI Dev (GPU)** | **Claym AI Dev (CPU)** |

## Dev Container の選択

VS Code でコンテナを開く際、GPU 版と CPU 版の選択ダイアログが表示されます。
...
両構成は同じ Dockerfile・スクリプトを共有しており、差分は `runArgs` の `--gpus=all` のみです。
